...
public class EventAeeMessageModule extends BotModule {

    public EventAeeMessageModule(GatewayDiscordClient gatewayDiscordClient, Configuration configuration) {
        super(gatewayDiscordClient, configuration);
    }

    @Override
    public void initialize() {
        super.gatewayDiscordClient.getChannelById(Channels.EVENTS_AEE.getId())
                .flatMap(channel -> {
                    if (!(channel instanceof MessageChannel messageChannel)) {
                        return Mono.error(new IllegalStateException("Le salon \"events_aee\" n'est pas un salon textuel !"));
                    }
                    ...
                    return messageChannel.getPinnedMessages().count().flatMap(aLong -> {
                        ...
                        return messageChannel.createMessage(EmbedCreateSpec.builder()
                                        .description("""
                                                Ce salon sera utilisé pour communiquer autour des événements organisés par l'AEE
                                                """)
